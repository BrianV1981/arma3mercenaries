...
// Check if _cas3Thread exists in the missionNamespace and terminate it if it does
if (!isNil {missionNamespace getVariable "_cas3Thread"} && {not isNull (missionNamespace getVariable "_cas3Thread")}) then {
    terminate (missionNamespace getVariable "_cas3Thread");
};
...
[
    "CAS",                         // CAS type
    [
        [27217, 24935, 0],                 // Position of vehicle (static)
...
missionNamespace setVariable ["_cas3Thread", [] spawn { 
    sleep 3600;
    ["WEST","CAS","F/A-181 Black Wasp II"] call ALiVE_fnc_combatSupportRemove;
}];
