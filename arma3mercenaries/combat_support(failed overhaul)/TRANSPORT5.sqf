...
*/
if (!isNil {missionNamespace getVariable "_transportThread5"} && {not isNull (missionNamespace getVariable "_transportThread5")}) then {
    terminate (missionNamespace getVariable "_transportThread5");
};
...
missionNamespace setVariable ["_transportThread5", [] spawn {
    sleep 7200;
    ["WEST","TRANSPORT","AW101 Merlin"] call ALiVE_fnc_combatSupportRemove;
}];
