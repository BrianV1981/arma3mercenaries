...
*/
if (!isNil {missionNamespace getVariable "_arty2Thread"} && {not isNull (missionNamespace getVariable "_arty2Thread")}) then {
    terminate (missionNamespace getVariable "_arty2Thread");
};
...
missionNamespace setVariable ["_arty2Thread", [] spawn { 
    sleep 3600;
    ["WEST","ARTY","Seara"] call ALiVE_fnc_combatSupportRemove;
}];
