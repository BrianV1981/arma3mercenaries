...
// Check if _transportThread14 exists and terminate it if it does
if (!isNil "_transportThread14" && {not isNull _transportThread14}) then {
    terminate _transportThread14;
};
...
private _safePos = [[27124,24585,0], 0, 1000, 10, 0, 20, 0] call BIS_fnc_findSafePos;
...
[
    ...
    [
        _safePos,                                                  // position
        60,                                                        // direction
        "B_Truck_01_medical_F",                                    // class
        "HEMTT Medical",                                           // callsign
