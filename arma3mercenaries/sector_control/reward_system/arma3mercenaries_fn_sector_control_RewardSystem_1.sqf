...
while {true} do {
    ...
    private _playersInSector = _markerPos nearEntities ["CAManBase", 125];
        ...
        if (isPlayer _x) then {
            ...
            for "_i" from 1 to 6 do {
               ...
               // Check if the player is still within 100m of the marker
				if ((_x distance _markerPos) > 125) then {
					["<t color='#FFFFFF' size='0.8'>You just left the sector!<br/>All payments have been haulted.<br/>Payments will resume in a few hours!</t>", -1, -1, 50, 1, 0, 789] remoteExec ["BIS_fnc_dynamicText", _x];
