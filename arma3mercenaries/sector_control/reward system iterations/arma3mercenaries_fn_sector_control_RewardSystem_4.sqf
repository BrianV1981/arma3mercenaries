...
// Initialize the block array and define block duration (in seconds)
private _blockedPlayers = [];
private _blockDuration = 300; // 5 minutes block time
...
while {true} do {
    ...
    // Cleanup old blocked players
    _blockedPlayers = _blockedPlayers select {time - (_x select 1) < _blockDuration};
    ...
    // Iterate over each player found in the sector
    {
        if (isPlayer _x) then {
            ...
            // Check if the player is already in the blocked array
            private _isBlocked = _blockedPlayers findIf {_x select 0 == _playerUID} != -1;
            if (_isBlocked) then {
                // Send a "you're blocked" message
                ["<t color='#ff0000' size='2'>You're blocked from capturing this sector!</t>", -1, -1, 4, 1, 0, 789] remoteExec ["BIS_fnc_dynamicText", _x, true];
                continue;
            };
            ...
            for "_i" from 1 to 6 do {
                ...
                if (_x distance _markerPos > 100) then {
                    ...
                    // Add the player to the blocked array with the current time
                    _blockedPlayers pushBack [_playerUID, time];
                    break;
                };
            ...
            // If the player received all 6 messages, add them to the blocked array
            _blockedPlayers pushBack [_playerUID, time];

            // Cleanup: Remove the capturing variable from the player
            _x setVariable [format ["isCapturingSector_%1", _uniqueSectorID], nil];
        };
