/*
    ...
    The script operates by continuously checking for virtual entities within a defined radius centered on the waypoint marker (`waypointMarker_1`). It evaluates the presence of entities affiliated with different factions (EAST, WEST, and GUER) and manages group activities accordingly. Each group that is spawned moves from their designated spawn point (`spawnMarker_1`) towards the waypoint, reacting based on the strategic importance of the waypoint location.
	
	This script also checks for any players within the defined radius and will despawn all groups if any player is detected.
    ...
    3. Ensure that the markers defined by `spawnMarker_1` and `waypointMarker_1` exist in your mission. The script utilizes `waypointMarker_1` as the focal point for checking the presence of ALiVE-mod virtual entities, influencing the dynamic control of spawning and despawning groups.
...
if (isServer) then {
    private _spawnMarkerName = "spawnMarker_1"; 
    private _waypointMarkerName = "waypointMarker_1"; 
    private _spawnPos = getMarkerPos _spawnMarkerName; 
    private _waypointPos = getMarkerPos _waypointMarkerName; 
    private _markerRadius = 100; // Radius for checking virtual entity presence
