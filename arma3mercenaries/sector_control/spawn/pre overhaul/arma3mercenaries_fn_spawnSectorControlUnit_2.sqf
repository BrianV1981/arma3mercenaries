/*
    ...
    The script operates by continuously checking for virtual entities within a 1000-meter radius centered on the waypoint marker (`waypointMarker_2`). It evaluates the presence of entities affiliated with different factions (EAST, WEST, and GUER) and manages group activities accordingly. Each group that is spawned moves from their designated spawn point (`spawnMarker_2`) towards the waypoint, reacting based on the strategic importance of the waypoint location.
    ...
    3. Ensure that the markers defined by `spawnMarker_2` and `waypointMarker_2` exist in your mission. The script utilizes `waypointMarker_2` as the focal point for checking the presence of ALiVE-mod virtual entities, influencing the dynamic control of spawning and despawning groups.
...
if (isServer) then {
    private _spawnMarkerName = "spawnMarker_2"; 
    private _waypointMarkerName = "waypointMarker_2"; 
    private _spawnPos = getMarkerPos _spawnMarkerName; 
    private _waypointPos = getMarkerPos _waypointMarkerName; 
    private _markerRadius = 200; // Radius for checking virtual entity presence
