/*
    ...
    The script operates by continuously checking for virtual entities within a 1000-meter radius centered on the waypoint marker (`waypointMarker_9`). It evaluates the presence of entities affiliated with different factions (EAST, WEST, and GUER) and manages group activities accordingly. Each group that is spawned moves from their designated spawn point (`spawnMarker_9`) towards the waypoint, reacting based on the strategic importance of the waypoint location.
    ...
    3. Ensure that the markers defined by `spawnMarker_9` and `waypointMarker_9` exist in your mission. The script utilizes `waypointMarker_9` as the focal point for checking the presence of ALiVE-mod virtual entities, influencing the dynamic control of spawning and despawning groups.
...
if (isServer) then {
    private _spawnMarkerName = "spawnMarker_9"; 
    private _waypointMarkerName = "waypointMarker_9"; 
