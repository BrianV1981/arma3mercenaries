/*
<details>
  <summary>fn_respawnsectorControl_independentEast_1.sqf</summary>
  
  fn_respawnsectorControl_independentEast_1.sqf
  ...
  - **Joint Respawn Logic:** If Independent or CSAT (East) controls the sector, both factions get a respawn point. If NATO (West) controls the sector, only NATO gets a respawn point.
  ...
  2. **Save the Script:**
     - Save the `fn_respawnsectorControl_independentEast_1.sqf` script in your mission's `scripts/sector_control/` folder.
  ...
  3. **Configure the Sector Control Module:**
     ...
     ```sqf
     [_this select 0, _this select 1, _this select 2] execVM 'scripts\sector_control\fn_respawnsectorControl_independentEast_1.sqf';
     ```
...
private _markerData = [
    ["respawnWest_sector_8", west],
    ["respawnIndependent_sector_8", independent],
    ["respawnEast_sector_8", east]
];
...
// Enable the respawn point for the faction that took control
{
    ...
    if (_ownerSide == _side || (_ownerSide == independent && _side == east) || (_ownerSide == east && _side == independent)) then {
