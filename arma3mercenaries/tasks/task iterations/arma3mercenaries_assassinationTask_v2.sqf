...
// Periodic check for the elimination of the HVT (every 5 seconds)
[
    _HVT, 
    _taskID, 
    _HVTGroup, 
    ...
    params ["_HVT", "_taskID", "_HVTGroup", "_guardGroup"];  // Receive the passed variables
    
    // Check every 5 seconds if the HVT is alive
    while {alive _HVT} do {
        sleep 5;
    };

    // Log task completion for debugging
    diag_log format ["Task ID: %1 marked as SUCCEEDED by periodic check", _taskID];

    // Task Completion (this ensures task state is updated in case the event handler doesn't catch the kill)
    [_taskID, "SUCCEEDED"] call BIS_fnc_taskSetState;
