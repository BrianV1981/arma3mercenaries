/*
	...
	Description:
    This script allows a BLUFOR or Independent player to "interrogate" captured units within a 30-meter radius.
    The nearest player to the object with the addAction is identified and tagged with a temporary variable.
    Each captured unit is assigned a unique variable and processed with a random health bleed between 1% and 10% damage.
     he health bleed ticks at a random interval between 10 and 30 seconds until the captured unit is dead, 
...
private _capturedUnits = _pos nearEntities ["Man", 30]; // Change 30 to any number of meters for the distance check
    ...
    [_x, _nearestPlayer, _uniqueID, _bleedPercentage, _capturedSideText, _playerFaction, _interrogationCompleteMessages] spawn {
        ...
        while {alive _cap && {_cap getVariable [_id, false]}} do {
            ...
            if (damage _cap >= 1) then {
                ...
                private _reward = 10000 + random 15000; // Random payout between 10,000 and 25,000 credits
