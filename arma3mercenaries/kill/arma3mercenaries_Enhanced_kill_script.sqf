    ...
    if (local _instigator && _killed isKindOf "CAManBase") then {
        ...
        [_killed, _instigator, _sideKilled, _sideKiller, _killed_Name, _kill_HUD, _killed_Color, _distance, _weaponDisplayName, _factionName] spawn {
            ...
            if (arma3mercenaries_killMarkerEnabled && isPlayer _instigator) then {
                ...
                // **Global Kill Marker Notification (Configurable via CBA)**
				if (arma3mercenaries_globalKillMarker) then {
					private _killFeedMessage = format ["%1 killed %2 with a %3 from %4m", name _instigator, _killed_Name, _weaponDisplayName, floor _distance];
            ...
            if (arma3mercenaries_deathMarkerEnabled && isPlayer _killed) then {
                ...
                // **Global Death Marker Notification (Configurable via CBA)**
				if (arma3mercenaries_globalDeathMarker) then {
					private _deathFeedMessage = format ["%1 was killed by %2 with a %3 from %4m", name _killed, name _instigator, _weaponDisplayName, floor _distance];
...
// Add event handler to all players to monitor unconscious state and death
{
    if (isPlayer _x && {alive _x}) then {
        _x addEventHandler ["HandleDamage", {
            ...
            // Check if unconscious marker feature is enabled
            if (arma3mercenaries_unconsciousMarkerEnabled) then {
                // Check if the player is unconscious or dead
                if (!alive _unit || [_unit] call ace_common_fnc_isAwake) then {
                    deleteMarker _mrkName;
                    // Reset the notification flag when the player regains consciousness or dies
                    _unit setVariable ["unconsciousNotified", false, true];
                } else {
                    // If the player is unconscious and not already notified
                    if (!_alreadyNotified) then {
                        // Create marker and send system chat message
                        deleteMarker _mrkName;
                        ...
                        // Set the flag to true to prevent multiple notifications
